...
use crate::entity::{token_info, token_summary, kline_5m, evt_trade_log};
use crate::utility::{LibError, LibResult};
use sea_orm::{EntityTrait, QueryFilter, ColumnTrait, QueryOrder, QuerySelect, Condition};

pub async fn get_basic_info(app_state: &AppState, token_address: &str) -> LibResult<schema::BasicInfoResp> {
    // 获取代币基本信息
...
pub async fn get_kline(
    ...
    let mut query = kline_5m::Entity::find()
        .filter(kline_5m::Column::TokenAddress.eq(token_address));
